...
function create(options) {
  return function(err, req, res, next) {
    options.model.forge().save(req.data).exec(next);
  };
}
...
function destroy(options) {
  return function(err, req, res, next) {
    options.model.forge({id: req.id}).destroy().exec(next);
  };
}
...
function search(options) {
   ...
   * @param {Integer} req.where.limit
   * @param {Integer} req.where.offset
   * @param {function} next
   */
  return function (err, req, res, next) {
